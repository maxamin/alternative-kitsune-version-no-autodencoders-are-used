...
# Dataset cleaning done with IQR found on In [3]
...
# using the scipy package to find the zscore
z_score = abs(dataset.apply(zscore))
#print(z_score.iloc[0,:])
#print(len(z_score))

# applying z score to every row of data in the dataset
for i in range(0, len(z_score)):
    for e in range(0, 115):
        if(z_score.iloc[i,e] > 3):
            z_score.drop([i])
            break
