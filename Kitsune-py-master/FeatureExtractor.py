...
# If wireshark is used then a csv file (parsed version of the pcap) will be made -which you can use as your input next time
class FE:
    ...
    def __prep__(self):
        ...
        ##If file is csv (pre-parsed by wireshark script)
        if type == "csv":
            self.parse_type = "csv"
        ...
        ##If file is pcap
            ...
            if os.path.isfile(self._tshark):
                self.pcap2csv_with_tshark()  # creates local csv file
                self.path += ".csv"
                self.parse_type = "csv"
            ...
            print("File: " + self.path + " is not a csv or pcap file")
            raise Exception()
        ...
        if self.parse_type == "csv":
            ...
            row = self.csvin.__next__() #move iterator past header
    ...
    def pcap2csv_with_tshark(self):
        ...
        cmd =  '"' + self._tshark + '" -r '+ self.path +' -T fields '+ fields +' -E header=n -E separator=, -E occurrence=f > '+self.path+".csv"
        subprocess.call(cmd,shell=True)
        print("tshark parsing complete. File saved as: "+self.path +".csv")
